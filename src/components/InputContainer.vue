<template>
  <v-toolbar elevation="3" color="white" :dense="this.$store.state.isRunningEmbedded" class="toolbar-content">
    ...
    <v-btn
      v-if="!shouldShowSendButton && !isModeLiveChat && !shouldShowUpload"
      @click="onMicClick"
    ...
    <v-btn
      v-if="shouldShowUpload && isUploadSlot"
...
export default {
  ...
  computed: {
    ...
    isUploadSlot() {
      return (this.$store.state.lex.sessionState?.dialogAction?.slotToElicit == 'AttachRenterDoc' || this.$store.state.lex.sessionState?.dialogAction?.slotToElicit == 'AttachOwnerDoc' || this.$store.state.lex.sessionState?.dialogAction?.slotToElicit == 'IdProof')
  ...
  methods: {
    ...
    onPickFile () {
      console.log("File selecter opened.....")
